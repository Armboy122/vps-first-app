...
CONTAINER_NAME="vps-first-app-prod"
...
# Print config
echo "🚀 เตรียมรัน production container (Yarn):"
...
echo "📥 กำลังดึง production image ล่าสุด..."
docker pull $FULL_IMAGE_NAME

# Start the container
echo "🚀 กำลังเริ่มต้น production container บนพอร์ต $PORT..."
docker run -d \
  --name $CONTAINER_NAME \
  -p $PORT:$DEFAULT_PORT \
  -e PORT=$DEFAULT_PORT \
...
echo "✅ Production container เริ่มทำงานแล้ว"
echo "📊 สามารถเข้าใช้งานได้ที่ http://localhost:$PORT"
echo "⚡ โหมด production ทำงานได้เร็วที่สุดและเสถียรที่สุด"
