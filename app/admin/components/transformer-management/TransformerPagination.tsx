...
export function TransformerPagination({ totalCount, totalPages }: TransformerPaginationProps) {
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= totalPages) {
      updateTransformerSearchParams({ page: newPage });
    }
  };
  ...
  const getPageNumbers = () => {
    ...
    for (
      let i = Math.max(2, transformerSearchParams.page - delta);
      i <= Math.min(totalPages - 1, transformerSearchParams.page + delta);
      i++
    ) {
      range.push(i);
    }

    if (transformerSearchParams.page - delta > 2) {
    ...
    if (transformerSearchParams.page + delta < totalPages - 1) {
  ...
  return (
    <div className="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
      ...
      <div className="flex flex-1 justify-between sm:hidden">
        <button
          onClick={() => handlePageChange(transformerSearchParams.page - 1)}
          disabled={transformerSearchParams.page <= 1}
        ...
        <button
          onClick={() => handlePageChange(transformerSearchParams.page + 1)}
          disabled={transformerSearchParams.page >= totalPages}
        ...
        <div>
          <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm">
            ...
            {getPageNumbers().map((pageNum, index) => {
              ...
              const isCurrentPage = pageNum === transformerSearchParams.page;
              return (
                <button
                  key={pageNum}
