...
LATEST_TAG="latest"
CACHE_FROM="$IMAGE_NAME:$LATEST_TAG"
...
echo " - $IMAGE_NAME:$DATE_TAG (with date)"
echo " - $IMAGE_NAME:$LATEST_TAG (latest version)"
echo "📝 To run the latest production image, use: ./run-prod.sh"
echo "📝 To run a specific version, use: ./run-prod.sh --tag $DATE_TAG" 