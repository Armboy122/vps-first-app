...
export const usePowerOutageFormLogic = ({
   ...
   */
  const handleTransformerSelect = useCallback((transformer: any) => {
  ...
  const onSubmit = useCallback(async (data: PowerOutageRequestInput) => {
    ...
    if (!validation.isValid) {
      setTimeError(validation.error || '');
      logError('power_outage_request_validation_failed', validation.error || 'Validation failed', { data });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        logError('power_outage_request_create_failed', result.error || 'Unknown error', { data });
        setSubmitStatus({
          success: false,
  ...
  const onAddToList = useCallback((data: PowerOutageRequestInput) => {
    ...
    if (!validation.isValid) {
      setTimeError(validation.error || '');
      logError('power_outage_request_add_validation_failed', validation.error || 'Validation failed', { data });
      return;
    }
