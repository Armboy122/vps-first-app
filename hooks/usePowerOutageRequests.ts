...
import { getPowerOutageRequests, updateOMS, updateStatusRequest, deletePowerOutageRequest } from '@/app/api/action/powerOutageRequest';
...
export const usePowerOutageRequests = () => {
  ...
  const createOutageDateFilter = useCallback((startDate: string, endDate: string, showPast: boolean) => {
    return (request: PowerOutageRequest) => {
  ...
  const filteredRequests = useMemo(() => {
    ...
    return filteredByFilters.filter((request) => {
      if (request.transformerNumber.toLowerCase().includes(lowercaseSearch)) return true;
      if (request.area?.toLowerCase().includes(lowercaseSearch)) return true;
      if (request.createdBy.fullName.toLowerCase().includes(lowercaseSearch)) return true;
      return false;
    });
  ...
  const currentItems = useMemo(() => {
    ...
    const lastItem = Math.min(firstItem + itemsPerPage, filteredRequests.length);
  ...
  // Actions
  const handleUpdateOMS = useCallback(async (id: number, status: OMSStatus) => {
    try {
  ...
  const handleUpdateStatus = useCallback(async (id: number, status: Request) => {
    try {
  ...
  const updatePagination = useCallback((options: Partial<PaginationOptions>) => {
  ...
  return {
    ...
    displayRange: {
      start: Math.min((currentPage - 1) * itemsPerPage + 1, filteredRequests.length),
